...
"""
def counts_dices(dices: list[int]) -> dict:
    """Подсчитывает частоты значений костей и возвращает словарь вида {значение: количество}."""
    counts = {}
    for num in dices:
...
def two_pairs_check(dices: list[int]) -> int:
...
def full_house_check(dices: list[int]) -> int:
